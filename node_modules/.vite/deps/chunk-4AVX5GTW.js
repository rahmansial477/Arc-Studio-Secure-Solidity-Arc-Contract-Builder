...
function hexToRlp(hex2, to2 = "hex") {
  return toRlp(hex2, to2);
}
...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k14 } = await import("./secp256k1-UWTV5S4Z.js");
  const signature_ = (() => {
...
var EstimateGasExecutionError = class extends BaseError {
  constructor(cause, { account, docsPath: docsPath3, chain: chain2, data, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to: to2, value }) {
    var _a;
...
function Field(ORDER, bitLen3, isLE = false, redef = {}) {
  if (ORDER <= _0n)
...
function numToSizedHex(num3, size4) {
  return bytesToHex3(numberToBytesBE(num3, size4));
}
var _0n3 = BigInt(0);
var _1n3 = BigInt(1);
var _2n2 = BigInt(2);
...
function weierstrassPoints(opts) {
  ...
  function isWithinCurveOrder(num3) {
    return inRange(num3, _1n3, CURVE.n);
  }
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey)
      num3 = mod(num3, N);
    aInRange("private key", num3, _1n3, N);
    return num3;
  }
  ...
  class Point4 {
     ...
     */
    static fromHex(hex2) {
      const P = Point4.fromAffine(fromBytes6(ensureBytes("pointHex", hex2)));
      P.assertValidity();
      return P;
    }
    ...
    toRawBytes(isCompressed = true) {
      ...
      this.assertValidity();
      return toBytes5(Point4, this, isCompressed);
    }
  ...
  const wnaf = wNAF(Point4, endo ? Math.ceil(nBitLength / 2) : nBitLength);
  return {
    CURVE,
...
function weierstrass(curveDef) {
  ...
  const bits2int_modN = CURVE.bits2int_modN || function(bytes) {
    return modN3(bits2int(bytes));
  };
  const ORDER_MASK = bitMask(nBitLength);
  function int2octets(num3) {
    aInRange("num < 2^" + nBitLength, num3, _0n3, ORDER_MASK);
...
function createHasher2(Point4, mapToCurve, defaults) {
  if (typeof mapToCurve !== "function")
    throw new Error("mapToCurve() must be defined");
  function map(num3) {
    return Point4.fromAffine(mapToCurve(num3));
  }
...
// node_modules/ox/_esm/core/internal/keyDerivation.js
...
// node_modules/ox/_esm/core/internal/mnemonic.js
...
var isPosBig = (n) => typeof n === "bigint" && _0n5 <= n;
...
function Field2(ORDER, bitLen3, isLE = false, redef = {}) {
  if (ORDER <= _0n6)
    throw new Error("invalid field: expected ORDER > 0, got " + ORDER);
  ...
  const f = Object.freeze({
    ...
    toBytes: (num3) => isLE ? numberToBytesLE2(num3, BYTES) : numberToBytesBE2(num3, BYTES),
    fromBytes: (bytes) => {
      if (bytes.length !== BYTES)
        throw new Error("Field.fromBytes: expected " + BYTES + " bytes, got " + bytes.length);
      return isLE ? bytesToNumberLE2(bytes) : bytesToNumberBE2(bytes);
    },
    // TODO: we don't need it here, move out to separate fn
...
function numToSizedHex2(num3, size4) {
  return bytesToHex4(numberToBytesBE2(num3, size4));
}
var _0n8 = BigInt(0);
var _1n8 = BigInt(1);
var _2n5 = BigInt(2);
...
function weierstrassPoints2(opts) {
  ...
  class Point4 {
     ...
     */
    static fromHex(hex2) {
      const P = Point4.fromAffine(fromBytes6(ensureBytes2("pointHex", hex2)));
      P.assertValidity();
      return P;
    }
    // Multiplies generator point by privateKey.
...
function weierstrass2(curveDef) {
  ...
  const slcNum = (b, from14, to2) => bytesToNumberBE2(b.slice(from14, to2));
  class Signature {
    constructor(r, s, recovery) {
    ...
    recoverPublicKey(msgHash) {
      ...
      if (radj >= Fp.ORDER)
        throw new Error("recovery id 2 or 3 invalid");
      const prefix = (rec & 1) === 0 ? "02" : "03";
      const R = Point4.fromHex(prefix + numToSizedHex2(radj, Fp.BYTES));
      const ir = invN(radj);
      const u1 = modN3(-h * ir);
      const u2 = modN3(s * ir);
      const Q = Point4.BASE.multiplyAndAddUnsafe(R, u1, u2);
  ...
  function int2octets(num3) {
    aInRange2("num < 2^" + nBitLength, num3, _0n8, ORDER_MASK);
    return numberToBytesBE2(num3, nByteLength);
  }
  function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
    if (["recovered", "canonical"].some((k) => k in opts))
      throw new Error("sign() legacy options not supported");
...
function expand_message_xmd2(msg, DST, lenInBytes, H) {
  ...
  const b = new Array(ell);
  const b_0 = H(concatBytes3(Z_pad, msg, l_i_b_str, i2osp2(0, 1), DST_prime));
  b[0] = H(concatBytes3(b_0, i2osp2(1, 1), DST_prime));
  for (let i = 1; i <= ell; i++) {
...
function lift_x2(x) {
  aInRange2("x", x, _1n9, secp256k1P2);
  const xx = modP2(x * x);
...
function schnorrVerify2(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange2(r, _1n9, secp256k1P2))
      return false;
    const s = num2(sig.subarray(32, 64));
    if (!inRange2(s, _1n9, secp256k1N2))
      return false;
...
var secp256k1_hasher2 = (() => createHasher3(secp256k13.ProjectivePoint, (scalars) => {
...
function normalize(fn) {
  afn(fn);
  return { encode: (from14) => from14, decode: (to2) => fn(to2) };
}
function convertRadix(data, from14, to2) {
  if (from14 < 2)
...
var radix2carry = (from14, to2) => from14 + (to2 - gcd(from14, to2));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from14, to2, padding2) {
  aArr(data);
  if (from14 <= 0 || from14 > 32)
    throw new Error(`convertRadix2: wrong from=${from14}`);
  if (to2 <= 0 || to2 > 32)
    throw new Error(`convertRadix2: wrong to=${to2}`);
  if (radix2carry(from14, to2) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from14} to=${to2} carryBits=${radix2carry(from14, to2)}`);
  }
  let carry = 0;
  let pos = 0;
...
var HDKey = class _HDKey {
  ...
  deriveChild(index) {
    ...
    try {
      if (this.privateKey) {
        const added = mod2(this.privKey + childTweak, secp256k13.CURVE.n);
  ...
  verify(hash2, signature) {
    ...
    let sig;
    try {
      sig = secp256k13.Signature.fromCompact(signature);
    } catch (error) {
      return false;
    }
...
function getSharedSecret(options) {
  ...
  const sharedPoint = point.multiply(secp256k12.utils.normPrivateKeyToScalar(from3(privateKey).slice(2)));
...
function verify(options) {
  ...
  return secp256k12.verify(options.signature, from2(payload), toBytes2(options.publicKey), ...hash2 ? [{ prehash: true, lowS: true }] : []);
}
...
async function fillTransaction(client, parameters) {
  var _a, _b, _c, _d, _e;
  const { account = client.account, accessList, authorizationList, chain: chain2 = client.chain, blobVersionedHashes, blobs, data, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce: nonce_, nonceManager: nonceManager2, to: to2, type, value, ...rest } = parameters;
  ...
  try {
    ...
    const format6 = ((_d = (_c = chain2 == null ? void 0 : chain2.formatters) == null ? void 0 : _c.transaction) == null ? void 0 : _d.format) || formatTransaction;
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-E46BPBHV.js").then((module) => module.WebSocket);
...
// node_modules/viem/_esm/utils/chain/filterChains.js
...
*/
//# sourceMappingURL=chunk-4AVX5GTW.js.map
